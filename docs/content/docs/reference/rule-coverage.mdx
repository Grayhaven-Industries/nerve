...
[modeling principles](/docs/concepts/harness-modeling), not from a governing document,
...
- **Crimp failure and wrong terminal.** HK-CONN-021 checks a terminal is
  ...
  [modeling principles](/docs/concepts/harness-modeling) warns about for designs that
...
(§38) names IPC/WHMA-A-620, IPC-D-620, NASA-STD-8739.4, and SAE-AS50881 as
intended inputs to future standards-informed rule packs, which is a roadmap
entry and not authority; the PRD does not name a revision, so the applicable revision is not
