...
async function createNextConfig(): Promise<NextConfig> {
  ...
  const nextConfig: NextConfig = {
    // This project sits inside the Nerve monorepo but is not a workspace
    // member, so it has its own lockfile. Without this, Next infers the
    // repository root and traces the wrong tree.
    turbopack: {
